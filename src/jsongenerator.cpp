...
struct Contact {
    ...
    bool validate() const {
        ...
        if (!regex_match(phone, phone_regex)) return false;
...
private:
    ...
    double similarity(const string& a, const string& b) const {
        ...
        if (a_low == b_low) return 1.0;
        
        size_t matches = 0;
        size_t min_len = min(a_low.size(), b_low.size());
        for (size_t i = 0; i < min_len; ++i) {
            if (a_low[i] == b_low[i]) ++matches;
        }
...
public:
    bool load(const string& filename) {
        ...
        while (getline(file, line)) {
            if (line.find("\"name\":") != string::npos) {
                ...
                if (getline(file, line)) extract_field(line, "\"phone\":", c.phone);
                if (getline(file, line)) extract_field(line, "\"email\":", c.email);
                if (getline(file, line) && line.find("\"tags\":") != string::npos) {
                    extract_tags(line, c.tags);
                }
                if (c.validate()) contacts.push_back(c);
            }
        }
    ...
    vector<Contact> find(const string& prefix, long long& time_ms) {
        ...
        result.erase(unique(result.begin(), result.end(), 
            [](const Contact& a, const Contact& b) { return a.name == b.name; }), result.end());
